$(function () {
    ...
    $("#btn_comment").click(function () {   // 오류를 찾아보자
...
function buildComment(result) {
    ...
    result.data.forEach(comment => {
        ...
        // 삭제버튼 여부  // 작성한 id 와 로그인한 id 가 같으면 버튼 보임 아님 빈 문자열
...
// 댓글 삭제 버튼이 눌렸을때, 해당 댓글을 삭제하는 이벤트를 등록
function addDelete(){
    ...
    $("[data-cmtdel-id]").click(function () {
        ...
        $.ajax({
            ...
            type: "POST",
            cache: false,
            data: {"id": comment_id}, // 삭제할 아이디를 담아서 보내쟈
            success: function(data, status){
                if (status == "success") {
                    if (data.status !== "OK") {
                        alert(data.status);
