...
public class OAuth2Controller {
    ...
    public KakaoOAuthToken kakaoAccessToken(String code) {
        ...
        try {
            token = mapper.readValue(response.getBody(), KakaoOAuthToken.class);    // (Json문자열, 변환할 타입)
            // AccessToken 확인
    ...
    public User registerKakaoUser (KakaoProfile profile){
       ...
       """.formatted(username, name, password, provider, providerId));
        ...
        if (user == null) { // 미가입자인 경우에만 회우너 가입 진행
            ...
            if (cnt > 0) {  //  정상적으로 insert 됬으면 1이 리턴 됐을꺼야
                System.out.println("[Kakao 인증 회원 가입 성공!]");
                user = userService.findByUsername(username);    // 다시 읽어온다 id, regdate 를 읽어오기 위해
            } else {
    ...
    //---------------------------------------------------
    // ■ 로그인 시키기
    public void loginKakaoUser(User kakaoUser) {    // Security 에 수동으로 로그인 처리
        UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                // username 과 password 를 이용해서...
